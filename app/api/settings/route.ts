  ...
  try {
    ...
    if (Array.isArray(results) && results.length > 0) {
      ...
      await query(
        ...
        [site_name || '', site_logo || '', site_favicon || '', site_favicon_dark || '', site_description || '', hero_title || 'Welcome to My Blog', hero_subtitle || 'Explore my thoughts on software, mechanics, and travels', hero_background || '', settingsId]
      );
    } else {
      // Insert new settings
